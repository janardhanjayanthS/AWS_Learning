...
![Launch Instance](../../assets/img/aws_ec2_home.png)
...
![Configure Instance](../../assets/img/aws_ec2_config.png)
...
### 4. Configure Nginx
- Configure nginx so that it redirects requests to the public IP of EC2 instance to the local host inside it where the app is running.

```bash
sudo vim /etc/nginx/sites-enabled/<any_name> # eg fastapi_nginx
```

- In the newly opened file paste the following, then save and quit the file.

```bash
server{
    listen 80; # port to listen on
    server_name <public IPv4 address from AWS console>;
    location / {
        proxy_pass http://127.0.0.1:8000;  # local host of the instance
    }
}
```

- Then restart the service with updated configs
```bash
...
- Then run the app using 'uvicorn main:app'

### 5. Run App as a Service (Persistent)
...
![Terminate Instance](../../assets/img/aws_ec2_terminate.png)
...
### Blog Posts & Guides
- [AWS EC2 Tutorial](https://medium.com/edureka/aws-ec2-tutorial-16583cc7798e) - A beginner's guide to Amazon EC2
- [Understanding Security Groups](https://www.geeksforgeeks.org/devops/what-is-security-group-in-aws-and-how-to-create-it/) - Security groups GFG
